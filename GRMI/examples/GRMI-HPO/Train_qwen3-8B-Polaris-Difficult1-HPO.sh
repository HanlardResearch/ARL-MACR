...
# 2  experiment_name
# 3  data.val_files
# 4  custom_reward_function.name
# 5  prompt_length
# 6  verl/workers/rollout/vllm_rollout/vllm_rollout_spmd.py
# This is run name
loss_mode=grpo
ROLE_NAME=SCI-Agent-v9  
MAX_ITER=3         

experiment_name=Qwen3-8B-$ROLE_NAME-$formatted_time
rolloutdir=output_dir/HPO/${formatted_day}/Train-${formatted_time}/rollouts
checkpointdir=/extrahome0/Research_HUB/verl/output_dir/HPO/${formatted_day}/Train-${formatted_time}/checkpoints
valrolloutdir=output_dir/HPO/${formatted_day}/Train-${formatted_time}/val-rollouts
log_path=output_dir/HPO/${formatted_day}/Train-${formatted_time}/$experiment_name.log
mkdir -p $rolloutdir
mkdir -p $checkpointdir
mkdir -p $valrolloutdir
export SWANLAB_MODE=cloud
export SWANLAB_LOG_DIR=output_dir/HPO/${formatted_day}/Train-${formatted_time}
...
python3 -m verl.trainer.main_ppo \
    ...
    custom_reward_function.path=verl/utils/reward_score/Cmath.py \
    custom_reward_function.name=compute_score4 \
    +actor_rollout_ref.rollout.extra.roles=$ROLE_NAME \
    +actor_rollout_ref.rollout.extra.role_config_path=data_dir/AgentRoles \
    ...
    data.train_files=data_dir/POLARIS/Polaris_train_filter.parquet \
    data.val_files="[ 'data_dir/gsm8k-style/aime2024.parquet', 'data_dir/gsm8k-style/aime2025.parquet' ]" \
