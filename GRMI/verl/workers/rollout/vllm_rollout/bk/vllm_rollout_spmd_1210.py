...
class vLLMRollout(BaseRollout):
    ...
    def post_process_for_static_length(self, input_bacth) -> torch.Tensor:
        ...
        return {
            ...
            "position_ids": processed_position_ids,
    ...
    def generate_sequences(
        ...
        """
        # if (os.environ.get("RANK", "0") == "0") or (torch.distributed.is_initialized() and torch.distributed.get_rank() == 0):
        #     print('>>>>>>>>>>>>>>>>>>>>Waiting for debugger<<<<<<<<<<<<<<<<<'); import debugpy; debugpy.listen(('localhost', 15678)); debugpy.wait_for_client()
        # prompts = prompts[:2]
        ...
        ####################################### <Agent-1> ###################################################
        # 第1轮 Generate Agent
        gen_prompts = [self._build_CMATHrole_zh_prompt_v3("generate", problem) for problem in problems]
        gen_input = self._encode_prompts_to_dataproto_with_maxLen(gen_prompts,
                                                      prompts.meta_info.copy(), self.config.prompt1_length)  # This is also gloabl prompt
        ...
        ####################################### <Agent-2> ###################################################
        # 第2轮 Review Agent
        rev_prompts = [self._build_CMATHrole_zh_prompt_v3("review", problem, answer) for problem, answer in
                       zip(problems, agent1_outputs)]
        rev_input = self._encode_prompts_to_dataproto_with_maxLen(rev_prompts, prompts.meta_info.copy(), self.config.prompt2_length)
        rev_output = self._generate_sequences(rev_input)
        agent2_outputs = self._decode_responses_wt_MaxLen(rev_output,self.config.response2_length)  # str
        ####################################### <Agent-2> ###################################################

        ####################################### <Agent-3> ###################################################
        # 第3轮 Improve Agent
        imp_prompts = [self._build_CMATHrole_zh_prompt_v3("improve", problem, answer, feedback) for problem, answer, feedback in
                       zip(problems, agent1_outputs, agent2_outputs)]
        imp_input = self._encode_prompts_to_dataproto_with_maxLen(imp_prompts, prompts.meta_info.copy(), self.config.prompt3_length)
        imp_output = self._generate_sequences(imp_input)
        agent3_outputs = self._decode_responses_wt_MaxLen(imp_output,self.config.response3_length)  # str
        ####################################### <Agent-3> ###################################################

        # if int(os.environ["RANK"]) == 0:
        #     torch.save(gen_prompts, "/code/tmp/debug/Agent1_input.pt")
        #     torch.save(agent1_outputs, "/code/tmp/debug/Agent1_output.pt")
        #     torch.save(rev_prompts, "/code/tmp/debug/Agent2_input.pt")
        #     torch.save(agent2_outputs, "/code/tmp/debug/Agent2_output.pt")
        #     torch.save(imp_prompts, "/code/tmp/debug/Agent3_input.pt")
        #     torch.save(agent3_outputs, "/code/tmp/debug/Agent3_output.pt")

        # 拼接 (Stitching) - 构建线性序列: [角色提示] | Pmt1 | Res1 | Pmt2 | Res2 | Pmt3 | Res3
        pmt1_transition_txt,pmt2_transition_txt,pmt3_transition_txt = self._build_CMATHrole_zh_prompt_v3(role="all")
        ...
        for i in range(batch_size):
            res1_ids = torch.tensor(tokenizer.encode(agent1_outputs[i], add_special_tokens=False), device=device)
            res2_ids = torch.tensor(tokenizer.encode(agent2_outputs[i], add_special_tokens=False), device=device)
            res3_ids = torch.tensor(tokenizer.encode(agent3_outputs[i], add_special_tokens=False), device=device)
            ...
            for token_ids, mask_val in global_response_parts:
                ...
                global_response_mask.append(torch.full((length,), mask_val, device=device))
        ...
        # # max_res_len between all ranks
        # local_max_res_len = torch.tensor(max_res_len, device=idx.device, dtype=torch.long)
        # torch.distributed.all_reduce(local_max_res_len, op=torch.distributed.ReduceOp.MAX)
        # max_res_len = local_max_res_len.item()
        # print(f'max_res_len:{max_res_len}')

        # static response length
        # max_res_len = self.config.response_length
        ...
        #  to [bs, res-len]
        batch_global_response_ids = torch.stack(batch_global_response_ids, dim=0)
        batch_global_response_mask = torch.stack(batch_global_response_mask, dim=0)

        # Agent-1 prompt is global prompt
        batch_global_prompt_ids = torch.cat([gen_input.batch["input_ids"].to(device),
                                             pmt1_transition_tensor.unsqueeze(0).expand(batch_size, -1)], dim=1)

        # # static prompt length
        # batch_global_prompt_ids = pad_sequence_to_length(
        #     batch_global_prompt_ids, max_seq_len=self.config.prompt_length, pad_token_id=self.pad_token_id, left_pad=True
        # ).to(device)
        ...
        batch_seq_ids = torch.cat([batch_global_prompt_ids, batch_global_response_ids.to(device)], dim=1)
        baTch_seq_mask = torch.cat([batch_global_prompt_mask, batch_global_response_mask.to(device)], dim=1)


        # ## static position id
        # res_position_ids = torch.arange(self.config.response_length, dtype=torch.long, device=device).unsqueeze(0).expand(batch_size, -1)
        # pmt_attention_mask = batch_global_prompt_mask
        # # pmt_response_mask = torch.zeros_like(pad_ids)
        # prompt_lengths = pmt_attention_mask.sum(dim=1).to(device)  # [B]
        # positions = torch.arange(self.config.prompt_length, device=device).unsqueeze(0)  # [1, L]
        # start_indices = (self.config.prompt_length - prompt_lengths.unsqueeze(1)).to(device)  # [B, 1]
        # relative_positions = positions - start_indices  # [B, L]
        # pmt_position_ids = torch.where(pmt_attention_mask.bool(), relative_positions, torch.zeros_like(relative_positions)).long()
        # last_prompt_pos = (pmt_position_ids * pmt_attention_mask).max(dim=1).values.to(device)  # [B]
        # res_offset = last_prompt_pos.unsqueeze(1) + 1  # [B, 1]
        # new_res_position_ids = (res_offset + res_position_ids).to(device)  # [B, L2]
        # batch_seq_position_ids = torch.cat([pmt_position_ids, new_res_position_ids], dim=1)  # [B, L1 + L2]
        ...
        response_position_ids = last_prompt_pos + torch.arange(1, res_len + 1 + pmt1_transition_tensor.size(-1),
                                                               device=last_prompt_pos.device).unsqueeze(
            0)  # [bs, res_len]
        batch_seq_position_ids = torch.cat([batch_global_prompt_position_ids, response_position_ids.to(device)], dim=1)

        ## 根据 self.config.prompt_length 和 self.config.response_length 静态长度后处理
        # 7. 构建最终 batch
        terdict = {
            "prompts": batch_global_prompt_ids,  # Global Prompt
            "responses": batch_global_response_ids,  # Global Response
            "input_ids": batch_seq_ids,
            "attention_mask": baTch_seq_mask,
            "position_ids": batch_seq_position_ids
        }
        terdict = self.post_process_for_static_length(terdict)

        # if int(os.environ["RANK"]) == 0:
        #     torch.save(terdict, "/code/tmp/debug/final_batch.pt")
    ...
    def _build_CMATHrole_en_prompt_v2(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        ...
        pmt0 = ('You are simulating the **Communication Mathematics Problem Solver System**, '
                'a unified multi-agent architecture for expert-level communication mathematics problem solving. '
        ...
        if role == "generate":
            ...
            raise ValueError(f"Unsupported role: {role}")
    def _build_CMATHrole_zh_prompt(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
        if role == "all":
            ...
            pmt2_transition_txt = (
                "[/PROPOSED SOLUTION]\n\n<|im_start|>user\n"
                "现在，您是[评审智能体]，请找出推理、计算或科学原理应用中的任何错误。\n"
                "<|im_end|>\n"
                "<|im_start|>assistant\n[REVIEW FEEDBACK]\n"
            )
            # Pmt3: 连接 Res2 和 Res3 (Improve 阶段指令)
            pmt3_transition_txt = (
                "[/REVIEW FEEDBACK]\n\n<|im_start|>user\n"
                "现在，您是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分。\n"
                "<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
            return pmt1_transition_txt, pmt2_transition_txt, pmt3_transition_txt
        ...
        pmt0 = ('你正在模拟来自 **鹏城实验室** 的**通信领域数学习题求解器系统**，'
                '这是一个用于专家级通信领域数学问题求解的统一多智能体架构。'
                '你将为给定问题**依次扮演三个不同的智能体角色**，且必须严格按以下顺序执行。\n\n'
                '对于每个角色，请严格遵循以下指示，并仅输出所要求的格式。\n\n---'
                '\n\n**[1. 生成智能体]**  '
                '\n你是一位专家级通信领域数学问题解决者。请提供一个**完整、严谨且自包含的解法**。'
                '以**逐步推理**的方式呈现你的思路，包括所有必要的定义、引理、逻辑推导和论证依据。'
                '不要自我质疑或含糊其辞——请以完全自信的语气书写。'
                '最后以一个清晰的 boxed 格式给出最终答案。'
                '\n\n**[2. 评审智能体]**  \n你是一位细致的数学评审专家。'
                '根据以下标准评估**所提出的解法**：  '
                '\n1. 逻辑正确性与内部一致性  '
                '\n2. 数学定义与定理的恰当使用  '
                ...
                '\n4. 计算或符号操作的准确性  '
                '\n\n请**严格**按以下格式之一进行回应：  '
                '\n- `<Pass>`：若解法完全正确且符合奥林匹克竞赛的严谨标准  '
                '\n- `<Fail>: [简洁、具体的理由]`：若存在任何缺陷（例如逻辑漏洞、错误断言、遗漏情况等）'
                '\n\n**[3. 改进智能体]**  '
                '\n你是一位优化专家。在已知**原始解法**和**评审反馈**的基础上，'
                '生成一个**修订后的解法**，需满足：  \n- 修正评审中指出的所有问题  '
                '\n- 保留原始推理中所有正确的部分  '
                '\n- 在必要处提升清晰度、严谨性或完整性  '
                '\n- 保持清晰的逐步推理结构  '
                '\n- 过程简洁避免重复和无效思考  '
                '\n\n仅输出**改进后的解法**——不要包含引言、元评论或标签。'
                )
        ...
        pmt1 = f"<|im_start|>system\n" \
               ...
               f"<|im_start|>user\n" \
               f"[PROBLEM]\n {clean_problem}\n[/PROBLEM]\n\n" \
               f"现在，你是[生成智能体]，请提供完整的解决方案，展示专家级的科学推理能力。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
        ...
        pmt2 = f"{pmt1}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"现在，您是[评审智能体]，请找出推理、计算或科学原理应用中的任何错误。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
        ...
        pmt3 = f"{pmt2}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"现在，您是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
    ...
    def _build_CMATHrole_zh_prompt_v2(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
        if role == "all":
            ...
            pmt2_transition_txt = (
                "[/PROPOSED SOLUTION]\n\n<|im_start|>user\n"
                "现在，你是[评审智能体]，请找出推理、计算或科学原理应用中的任何错误。\n" \
                "<|im_end|>\n"
                "<|im_start|>assistant\n[REVIEW FEEDBACK]\n"
            )
            # Pmt3: 连接 Res2 和 Res3 (Improve 阶段指令)
            pmt3_transition_txt = (
                "[/REVIEW FEEDBACK]\n\n<|im_start|>user\n"
                "现在，你是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分，最后以标准格式给出答案。\n" 
                "<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
            return pmt1_transition_txt, pmt2_transition_txt, pmt3_transition_txt
        ...
        pmt0 = ('你正在模拟专家级的**通信领域数学习题求解器系统**，'
                ...
                '以**逐步推理**的方式呈现你的思路，包括所有必要的定义、引理、逻辑推导和论证依据。'
                '不要自我质疑或含糊其辞——请以完全自信的语气书写。'
                '最后以一个清晰的 boxed 格式给出最终答案。判断题用\\text{是}或\\text{否}回答，非精确数值保留2位小数。'
                '\n\n**[2. 评审智能体]**  \n你是一位细致的数学评审专家。'
                '根据以下标准评估**所提出的解法**：  '
                '\n1. 逻辑正确性与内部一致性  '
                '\n2. 数学定义与定理的恰当使用  '
                '\n3. 论证的完整性（无未经证明的跳跃）  '
                '\n4. 计算或符号操作的准确性以及是否使用正确格式给出答案  '
                '\n\n请**严格**按以下格式之一进行回应：  '
                '\n- `<Pass>`：若解法完全正确且符合标准答案格式  '
                '\n- `<Fail>: [简洁、具体的理由]`：若存在任何缺陷（例如逻辑漏洞、错误断言、数值或格式错误等）'
                '\n\n**[3. 改进智能体]**  '
                '\n你是一位优化专家。在已知**原始解法**和**评审反馈**的基础上，'
                '生成一个**修订后的解法**，需满足：  \n- 修正评审中指出的所有问题  '
                '\n- 保留原始推理中所有正确的部分  '
                ...
                '\n- 保持清晰的逐步推理结构  '
                '\n- 过程简洁避免重复和无效思考  '
                '\n\n仅输出**改进后的解法**——不要包含引言、元评论或标签， 最后以标准格式给出答案。 '
                )
        ...
        pmt2 = f"{pmt1}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"现在，你是[评审智能体]，请找出推理、计算或科学原理应用中的任何错误。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
        ...
        pmt3 = f"{pmt2}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"现在，你是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分，最后以标准格式给出答案。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
    ...
    def _build_CMATHrole_zh_prompt_v3(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
        ...
        key1 = "现在，你是[数学领域专家]，请一步一步地给出问题的求解步骤，最后以一个清晰的 boxed 格式给出最终答案。判断题用\\text{是}或\\text{否}回答，非精确数值保留2位小数。过程尽可能简洁明了。\n"

        key2 = "现在，你是[通信领域专家]，请根据你的专业知识判断求解过程中是否存在违背通信科学原理的错误。\n" \
               '\n\n请**严格**按以下格式之一进行回应：  ' \
               '\n- `<Pass>`：若解法完全正确，不违背任何通信原理，符合标准答案格式  ' \
               '\n- `<Fail>: [简洁、具体的理由]`：若存在任何缺陷（例如逻辑漏洞、原理或概念错误、数值或格式错误等）'
        key3 = "现在，你是[求解步骤优化专家]，请根据[通信领域专家]的反馈对[数学领域专家]的求解步骤进行逐步核对，修改错误，最后以标准格式给出答案。\n"
        ...
        pmt0 = '你正在模拟专家级的**通信领域数学习题求解器系统**，你具备通信和数学领域的专家级知识。'
    ...
    def _build_CMATHrole_en_prompt(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
        if role == "all":
            ...
            pmt3_transition_txt = (
                "[/REVIEW FEEDBACK]\n\n<|im_start|>user\n"
                f"Please revise solutions based on specific review feedback while preserving correct elements.\n"
                "<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
            return pmt1_transition_txt, pmt2_transition_txt, pmt3_transition_txt
        ...
        pmt2 = f"{pmt1}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"Please identify any errors in reasoning, calculations, or application of scientific principles.\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
    ...
    def _build_CMATHrole_en_prompt_v3(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
        if role == "all":
            ...
            pmt2_transition_txt = (
                "[/PROPOSED SOLUTION]\n\n<|im_start|>user\n"
                "Please identify any errors in reasoning, calculations, numerical or formatting error, and application of scientific principles.\n"
                "<|im_end|>\n"
                "<|im_start|>assistant\n[REVIEW FEEDBACK]\n"
            )
            # Pmt3: 连接 Res2 和 Res3 (Improve 阶段指令)
            pmt3_transition_txt = (
                "[/REVIEW FEEDBACK]\n\n<|im_start|>user\n"
                "Please revise solutions based on specific review feedback while preserving correct elements, and finally present the answer in standard format. \n"
                "<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
            return pmt1_transition_txt, pmt2_transition_txt, pmt3_transition_txt
        ...
        pmt1 = f"<|im_start|>system\n" \
               ...
               f"<|im_start|>user\n" \
               f"[PROBLEM]\n {clean_problem}\n[/PROBLEM]\n\n" \
               "Please reason step by step, and put your final answer within \\boxed{}. Answer true/false questions with \\text{是} or \\text{否}, and round non-exact numerical values to two decimal places." \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
        ...
        pmt2 = f"{pmt1}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"Please identify any errors in reasoning, calculations, numerical or formatting error, and application of scientific principles.\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
        ...
        pmt3 = f"{pmt2}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"Please revise solutions based on specific review feedback while preserving correct elements, and finally present the answer in standard format. \n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
    ...
    def _build_CMATHrole_zh_prompt_v4(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        # ... (保持原有的 _build_role_prompt 不变) ...
        # 注意：这个函数用于生成独立的 input_ids 给模型推理，
        # 而 Pmt2/Pmt3 的拼接文本是在 generate_sequences 内部手动构建的。
        """构建 SciAgent 风格的角色化 prompt"""
