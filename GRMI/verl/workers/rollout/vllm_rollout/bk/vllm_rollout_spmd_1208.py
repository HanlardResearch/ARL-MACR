...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(
        ...
        ####################################### <Agent-1> ###################################################
        # 第1轮 Generate Agent
        gen_prompts = [self._build_CMATHrole_zh_prompt_v2("generate", problem) for problem in problems]
        ...
        ####################################### <Agent-2> ###################################################
        # 第2轮 Review Agent
        rev_prompts = [self._build_CMATHrole_zh_prompt_v2("review", problem, answer) for problem, answer in
        ...
        ####################################### <Agent-3> ###################################################
        # 第3轮 Improve Agent
        imp_prompts = [self._build_CMATHrole_zh_prompt_v2("improve", problem, answer, feedback) for problem, answer, feedback in
        ...
        pmt1_transition_txt,pmt2_transition_txt,pmt3_transition_txt = self._build_CMATHrole_zh_prompt_v2(role="all")
    ...
    def _build_CMATHrole_zh_prompt_v2(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        ...
        if role == "all":
            ...
            pmt3_transition_txt = (
                "[/REVIEW FEEDBACK]\n\n<|im_start|>user\n"
                "现在，您是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分，最后以标准格式给出答案。\n" 
                "<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
            return pmt1_transition_txt, pmt2_transition_txt, pmt3_transition_txt
        ...
        pmt3 = f"{pmt2}" \
               ...
               f"<|im_end|>\n" \
               f"<|im_start|>user\n" \
               f"现在，您是[改进智能体]，请根据具体的审阅反馈修订解决方案，同时保留正确的部分，最后以标准格式给出答案。\n" \
               f"<|im_end|>\n" \
               f"<|im_start|>assistant\n"
