...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(
        ...
        gen_input = self._encode_prompts_to_dataproto(gen_prompts, prompts.meta_info.copy()) # This is also gloabl prompt
        gen_output = self._generate_sequences(gen_input)
        agent1_outputs = self._decode_responses(gen_output)
        ####################################### <Agent-1> ###################################################

        ####################################### <Agent-2> ###################################################
        # 第2轮 Review Agent
        rev_prompts = [self._build_role_prompt("review", problem, answer)  for problem, answer in zip(problems, agent1_outputs)]
        ...
        agent2_outputs = self._decode_responses(rev_output) # str
        ####################################### <Agent-2> ###################################################

        ####################################### <Agent-3> ###################################################
        # 第3轮 Improve Agent
        imp_prompts = [self._build_role_prompt("improve", problem, answer, feedback)  for problem, answer, feedback in zip(problems, agent1_outputs, agent2_outputs)]
        ...
        agent3_outputs = self._decode_responses(imp_output)
        ####################################### <Agent-3> ###################################################
        ...
        pmt1_transition_tensor = torch.tensor(tokenizer.encode(pmt1_transition_txt, add_special_tokens=False), device=device)
        ...
        max_seq_len = max_res_len + batch_global_prompt_ids.shape[-1]
        assert max_seq_len < self.config.response_length + self.config.prompt_length
        ...
        response_position_ids = last_prompt_pos + torch.arange(1, res_len + 1 + pmt1_transition_tensor.size(-1), device=last_prompt_pos.device).unsqueeze(
            0)  # [bs, res_len]
