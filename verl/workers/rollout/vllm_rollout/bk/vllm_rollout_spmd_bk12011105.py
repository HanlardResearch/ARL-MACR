...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(
        ...
        # 4. 迭代执行 Review-Improve 轮次
        for round_num in range(max_rounds):
            # 准备需要 review 的样本索引
            review_indices = [i for i in range(batch_size) if not completed[i]]
            ...
            for idx_pos, sample_idx in enumerate(review_indices):
                ...
                # # 检查是否通过 review
                # if "<Pass>" in rev_feedbacks[idx_pos]:
                #     completed[sample_idx] = True
                #     continue

            # 4.2 准备需要 improve 的样本索引
            imp_indices = [i for i in review_indices if not completed[i]]
            if not imp_indices:
                continue
            ...
            # 5. 拼接 (Stitching) - 构建线性序列: Pmt1 | Res1 | Pmt2 | Res2 | Pmt3 | Res3
    ...
    def _build_role_prompt(self, role: str, problem: str, content: str = "", feedback: str = "") -> str:
        ...
        pmt0 = ('You are simulating the **Math Olympiad Worker System** from **SciAgent**, '
                ...
                # '\n\n---\n\nBegin now with the **Generate Agent** for the following problem:  '
                # '\n\n> **Problem:** {clean_problem}  '
                # '\n\nProceed through all three roles in sequence within a single response, clearly separated as:  '
                # '\n\n```'
