...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(
        ...
        ####################################### <Agent-2> ###################################################
        # 第2轮 Review Agent
        rev_prompts = [self._build_CMATHrole_zh_prompt_selfdefine("review", problem, answer) for problem, answer in
        ...
        ####################################### <Agent-3> ###################################################
        # 第3轮 Improve Agent
        imp_prompts = [self._build_CMATHrole_zh_prompt_selfdefine("improve", problem, answer, feedback) for problem, answer, feedback in
    ...
    def _build_CMATHrole_zh_prompt_v6(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        ...
        key2 =  ('\n\n现在你是一位细致的[数学评审专家]兼[通信领域专家]。'
                '根据以下标准评估**原始解法**：  '
                '\n1. 逻辑正确性与内部一致性  '
                '\n2. 数学定义与定理的恰当使用  '
                '\n3. 求解过程中是否存在违背通信科学原理的错误  '
                '\n4. 计算或符号操作的准确性  '
                '\n5. 结果的格式检查，判断题用\\text{是}或\\text{否}回答，非精确数值保留2位小数 '
                '\n\n请**严格**按以下格式之一给出**评审反馈**：  '
                '\n- `<Pass>`：若解法完全正确，不违背任何通信原理和数学知识，符合标准答案格式   '
                '\n- `<Fail>: [简洁、具体的理由]`：若存在任何缺陷（例如逻辑漏洞、原理或概念错误、数值或格式错误等）'
                 )

        key3 = ('\n现在你是一位[解题步骤优化]专家。在已知**原始解法**和**评审反馈**的基础上，'
                '生成一个**修订后的解法**，需满足：  \n- 修正评审中指出的所有问题  '
                '\n- 保留原始推理中所有正确的部分  '
                '\n- 在必要处提升清晰度、严谨性或完整性  '
                '\n- 保持清晰的逐步推理结构  '
    ...
    def _build_CMATHrole_zh_prompt_selfdefine(self, role: str, problem: str = "", content: str = "", feedback: str = "") -> str:
        ...
        key1,key2,key3 = get_key_prompts(self.config.extra['roles'])
