...
class vLLMRollout(BaseRollout):
    ...
    @torch.no_grad()
    def _get_response_mask_for_sciagent(self, all_responses: List[List[int]], eos_token_id: int) -> torch.Tensor:
        """
        ...
        return torch.tensor(padded_mask, dtype=torch.long, device=self.device)
    ...
    def generate_sequences(
        ...
        """
        模拟 SciAgent 多智能体协作流程，生成符合原始 generate_sequences 格式的 DataProto
        """
        # 原始输入
        ...
        # 仅支持 batch_size=1
        if batch_size != 1:
            raise NotImplementedError(f"SciAgent collaboration currently only supports batch_size=1 ({idx.shape})")
        # 提取原始 prompt
        prompt_token_ids = _pre_process_inputs(self.pad_token_id, idx[0])
        problem = self.model_config.tokenizer.decode(prompt_token_ids)

        # 初始化存储
        all_responses = []  # List[List[int]] - 每个角色生成的token IDs
        role_sequence = ["generate"]  # 记录角色顺序

        # Step 1: Generate
        gen_prompt = self._build_role_prompt("generate", problem)
        gen_input = self._encode_prompts_to_dataproto([gen_prompt], prompts.meta_info.copy())
        gen_output = self._generate_sequences(gen_input)
        gen_resp = gen_output.batch["responses"][0]
        gen_resp_clean = gen_resp[gen_resp != self.pad_token_id].tolist()
        all_responses.append(gen_resp_clean)
        current_answer = self._decode_responses(gen_output)[0]

        # Step 2: 迭代 Review-Improve
        for _ in range(max_rounds):
            # Review
            rev_prompt = self._build_role_prompt("review", problem, current_answer)
            rev_input = self._encode_prompts_to_dataproto([rev_prompt], prompts.meta_info.copy())
            rev_output = self._generate_sequences(rev_input)
            rev_resp = rev_output.batch["responses"][0]
            rev_resp_clean = rev_resp[rev_resp != self.pad_token_id].tolist()
            all_responses.append(rev_resp_clean)
            role_sequence.append("review")
            review_feedback = self._decode_responses(rev_output)[0]

            if "<Pass>" in review_feedback:
                break

            # Improve
            imp_prompt = self._build_role_prompt("improve", problem, current_answer, review_feedback)
            imp_input = self._encode_prompts_to_dataproto([imp_prompt], prompts.meta_info.copy())
            imp_output = self._generate_sequences(imp_input)
            imp_resp = imp_output.batch["responses"][0]
            imp_resp_clean = imp_resp[imp_resp != self.pad_token_id].tolist()
            all_responses.append(imp_resp_clean)
            role_sequence.append("improve")
            current_answer = self._decode_responses(imp_output)[0]

        # 构建多段式 response_mask
        response_mask = self._get_response_mask_for_sciagent(all_responses, eos_token_id)

        # 构建 responses tensor (所有生成内容拼接)
        flattened_responses = [token for resp in all_responses for token in resp]
        max_resp_len = self.config.response_length
        if len(flattened_responses) > max_resp_len:
            flattened_responses = flattened_responses[:max_resp_len]

        # 填充到 config.response_length
        padded_responses = flattened_responses + [self.pad_token_id] * (max_resp_len - len(flattened_responses))
        responses_tensor = torch.tensor(padded_responses, dtype=torch.long, device=idx.device)
        responses_tensor = responses_tensor.unsqueeze(0)  # (1, response_length)
        response_mask = response_mask.unsqueeze(0)  # (1, response_length)

        # 构建 attention_mask - 多段式
        # 格式: | prompt | generate | review | improve | ... | padding |
        # 注意: 原始实现中 prompt 是 left-padded，所以 attention_mask 前面是0
        prompt_attention = attention_mask[0]  # (prompt_length,)
        response_attention = get_response_mask(
            response_id=responses_tensor,
            eos_token=eos_token_id,
            dtype=attention_mask.dtype
        )
        final_attention_mask = torch.cat([prompt_attention, response_attention[0]], dim=0).unsqueeze(0)
        ...
        # 构建 input_ids (prompt + responses)
        input_ids = torch.cat([idx[0], responses_tensor[0]], dim=0).unsqueeze(0)

        # 构建最终 batch
        final_batch = TensorDict({
            "prompts": idx,
            "responses": responses_tensor,
            "response_mask": response_mask,  # 新增字段，符合文档但原代码未实现
            "input_ids": input_ids,
