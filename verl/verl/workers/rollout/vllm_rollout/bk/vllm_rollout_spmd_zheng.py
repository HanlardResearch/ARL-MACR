...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(
        ...
        encoded_pmt1 = self.model_config.tokenizer.encode(pmt1_transition_txt, add_special_tokens=False)
        pmt1_tensor = torch.tensor(encoded_pmt1, dtype=gen_input.batch["input_ids"].dtype, device=device)
        pmt1_expanded = pmt1_tensor.unsqueeze(0).expand(batch_size, -1)
        unpad_input = torch.cat([gen_input.batch["input_ids"].to(device), pmt1_expanded], dim=1)
        pad_ids = pad_sequence_to_length(
            unpad_input, max_seq_len=unpad_input.size, pad_token_id=self.pad_token_id, left_pad=True
        ).to(device)
        ...
        pmt_attention_mask = (pad_ids != self.pad_token_id).int().to(device)
        ...
        print("start_indices.device:", start_indices.device)
        ...
        # 将 response 相对位置偏移到 prompt 之后
        res_offset = last_prompt_pos.unsqueeze(1) + 1  # [B, 1]
        new_res_position_ids = (res_offset + res_position_ids).to(device)  # [B, L2]
        final_position_ids = torch.cat([pmt_position_ids, new_res_position_ids], dim=1)  # [B, L1 + L2]

        for i in range(batch_size):
            # 1. 获取各个片段
            # Pmt1 (去除 padding)
            # p1_ids = gen_input.batch[i]["input_ids"].to(idx.device)
            # p1_ids = idx[i]
            ...
            # 2. 构建线性序列 (Input IDs)
            # 结构: [Pmt1] + [Res1] + [Pmt2] + [Res2] + [Pmt3] + [Res3]
            ...
            for token_ids, type_id in parts:
                ...
                mask_val = 1 if type_id == 1 else 0
                ...
                if type_id == 1:
                    boundaries.append((current_idx + last_prompt_pos[i], current_idx + length + last_prompt_pos[i]))
        ...
        # 初始化 Tensors
        # max_seq_len = self.config.prompt_length + self.config.response_length
        padded_input_ids = torch.full((batch_size, self.config.response_length), self.pad_token_id, dtype=torch.long, device=device)
        padded_response_mask = torch.zeros((batch_size, self.config.response_length), dtype=torch.bool, device=device)
        padded_attention_mask = torch.zeros((batch_size, self.config.response_length), dtype=torch.long, device=device)
        ...
        final_attention_mask = torch.cat([pmt_attention_mask, padded_attention_mask], dim=1)
        ...
        # position_ids = torch.arange(max_seq_len, dtype=torch.long, device=device).unsqueeze(0).expand(batch_size,-1)
        ...
        final_batch = TensorDict({
            ...
            "input_ids": torch.cat([pad_ids, responses_tensor], dim=-1),
